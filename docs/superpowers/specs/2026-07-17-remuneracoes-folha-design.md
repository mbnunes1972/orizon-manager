...
### Atribuição — "quem executa cada projeto" (onde mora + visão completa)
**Onde mora:** tabela **`atribuicoes_ambiente`** (por projeto/ambiente): `papel` (projeto_executivo/medicao/montagem/assistencia) → `funcionario_id`/`terceiro_id`. Editada na modal **"Mapa de Atribuições"** do Ciclo (`GET/POST /api/projetos/<nome>/atribuicoes`), resolvida por `mod_escopo.resolver_responsavel`; cada **etapa do ciclo** já resolve `responsavel_efetivo` por **etapa → papel → executor**.
**Visão completa (extensão futura, confirmada com o usuário):** ao **concluir uma etapa** do ciclo, o sistema identifica o **serviço** (etapa→papel), o **Mapa** dá o **executor**, e a **comissão** desse papel é calculada (base do projeto × comissão da função) e **constituída**. Hoje só o **Consultor** (venda, `criado_por_id`) calcula automático; os demais papéis entram pela **base editável** (manual) — o gatilho automático (Mapa → base por papel na conclusão da etapa) é a extensão.
**Contábil (confirmado):** paga simples (motor atual) = **débito Despesa 5.3.x × crédito Caixa/Banco 1.1.01**. Modelo completo de matching (à la FASE D2 da venda): **constituir** na conclusão da fase (débito Despesa / Ativo diferido × crédito Passivo "comissões a pagar" 2.x) e **baixar** contra Caixa na paga — daí o ciclo tocar **ativo, passivo, caixa/bancos e despesa**. Contas exatas + tratamento no contra-cheque → **com o contador**.
