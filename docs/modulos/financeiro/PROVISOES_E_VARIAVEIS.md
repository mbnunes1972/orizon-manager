...
| Provisão de Montagem (visão) | `Prov_Mont` | `%montagem_pct × Val_Cont` — fold FASE 2 |
| Provisão de Garantia (visão) | `Prov_Gar` | `%garantia_pct × Val_Cont` — fold FASE 2 |
...
> **Fold FASE 2 (Montagem/Garantia na aprovação):** `Prov_Mont`/`Prov_Gar` = `% × Val_Cont`, MESMA base
> e mesmo arredondamento da constituição contábil no fechamento (`5.6.x × 2.1.04.x`). É só **VISÃO** — a
