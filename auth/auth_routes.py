    ...
    if path == "/api/auth/me":
        ...
        if not usuario:
            ...
            try:
                ...
                # PDV (loja com mãe, spec 2026-07-22): o painel Financeiro não aparece na UI
                # do PDV — o financeiro é operado pela mãe (visão unificada). Só a TELA some:
                # modulo_ativo() segue True, então o wiring de lançamentos do razão continua.
                if _loja_ativa is not None and getattr(_loja_ativa, "loja_mae_id", None):
                    _ativos = _ativos - {"financeiro"}
