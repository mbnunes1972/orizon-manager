...
def test_matriz_deriva_de_perfis():
    ...
    d = next(x for x in m["perfis"] if x["slug"] == "diretor")
    ...
    assert cons["capacidades"] == [] and cons["desconto_max"] == 10.0
...
def test_perfis_matriz_endpoint_gate(http_client_factory, seed):
    ...
    assert any(p["slug"] == "medidor" for p in d["perfis"])
    assert "autorizar" in d["capacidades"]
