...
> `## Sessão 111`. **PENDENTE:** push das DUAS branches (credencial GitHub só funciona fora
> desta sessão — `!git push` também falhou pelo GCM do Windows; ver Sessão 111), merge na `main`
> (PDV primeiro, faxina em cima), deploy A/B (atenção: re-login geral pelo cookie novo + DROP
> das colunas omie_* ao subir), re-ingestão do grafo após merge. Recomendação registrada:
> `git rm --cached orizon.db` numa frente própria.
>
...
> vivos). Ver `## Sessão 110` e o spec `_geral/2026-07-22-ponto-de-venda-design.md`.
...
## Sessão 111 — FAXINA: Omie removido por inteiro + PostgreSQL como único banco (fim do SQLite)
...
## Sessão 110 — Ponto de Venda (PDV avançado): frente completa em 4 fatias
