...
**Débito registrado (não recolorido nesta frente):** há ~16 botões de ação (Aprovar/Confirmar do Ciclo/Contrato/Medição) com `#b8960c` dourado literal (`.btn-amber`/`.btn-ciclo` + inline) — violam "sempre o token", e o v9 diz que dourado não é p/ botão de ação. Recolori-los é mudança visual transversal (semântica de aprovação) → aguardando decisão do usuário (tokenizar p/ `var(--gold)` mantendo dourado × converter p/ primário/accent × deixar p/ passe dedicado).
